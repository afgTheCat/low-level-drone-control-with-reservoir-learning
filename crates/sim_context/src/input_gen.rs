...
mod test {
    use crate::{
        input_gen::{build_data_set, InputGenerator},
        SimContext,
    };
    use flight_controller::Channels;
    use std::time::Duration;
    ...
    #[test]
    fn build_200_test_data() {
        build_data_set("200_test_data".into(), Duration::from_secs(5), 0, 200);
    }
    ...
    // This should generate 5 dataset groups, each with 3 datasets:
    // a 5 long, a 10 long, and a 25 long (both training and testing),
    // where each subsequent data set is a superset of the previous.
    #[test]
    fn build_25_inc() {
        fn simulate_episodes(
            ...
            data_set_id: &str,
            prefix: &str,
            episodes: &[Vec<Channels>],
        ) {
            for (ep, inputs) in episodes.iter().enumerate() {
                let simulation_id = format!("{data_set_id}/{prefix}_{ep}");
                context.set_logger(crate::LoggerType::File(simulation_id));
                let mut simulation = context.try_load_simulator().unwrap();
                simulation.init();
                for &input in inputs {
        ...
        let input_generator = InputGenerator::default()
            .set_throttle(super::InputGenerationMethod::Brownian)
            .set_yaw(super::InputGenerationMethod::Brownian)
            .set_pitch(super::InputGenerationMethod::Brownian)
            .set_roll(super::InputGenerationMethod::Brownian);

        let training_inputs = (0..25usize)
            .map(|_| input_generator.generate(duration))
            .collect::<Vec<_>>();
        let test_inputs = (0..25usize)
            .map(|_| input_generator.generate(duration))
            .collect::<Vec<_>>();

        for &len in &[5usize, 10usize, 25usize] {
            let data_set_id = format!("{len}_len_inc");
            simulate_episodes(
                &mut context,
                &data_set_id,
                "training",
            ...
            simulate_episodes(&mut context, &data_set_id, "testing", &test_inputs[..len]);
        }
    }

    #[test]
    fn build_intermediate_dataset() {
        build_data_set(
            "intermediate_dataset".into(),
